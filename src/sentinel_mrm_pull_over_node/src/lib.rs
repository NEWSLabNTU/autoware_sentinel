...
//! Sentinel `/system/mrm_pull_over_manager`.
//!
...
pub struct PullOverManagerNode;

impl Node for PullOverManagerNode {
    const NAME: &'static str = "mrm_pull_over_manager";

    fn register(ctx: &mut NodeContext<'_>) -> NodeResult<()> {
        ensure_island_default();
        let mut node =
            ctx.create_node(NodeOptions::new("mrm_pull_over_manager").namespace("/system"))?;
        node.create_publisher_for_topic::<MrmBehaviorStatus>(
            "/system/mrm/pull_over_manager/status",
        )?;
        node.create_service_server_for_name::<OperateMrm>("/system/mrm/pull_over_manager/operate")?;
        node.create_timer_for_callback_name("on_tick", TimerDuration::from_millis(33))?;
        Ok(())
    }
}

impl ExecutableNode for PullOverManagerNode {
    type State = ();
    fn init() -> Self::State {}
