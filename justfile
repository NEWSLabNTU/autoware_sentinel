...
build-spe-firmware: orin_spe-bsp-stage
    ...
    NV_SPE_FSP_DIR="$PREFIX" \
        ...
        ZPICO_MAX_PENDING_GETS=1 \
        ZPICO_SUBSCRIBER_RING_DEPTH=2 \
...
cross-check:
    ...
    cargo check --workspace --exclude autoware_sentinel_linux --exclude autoware_sentinel_core --exclude ivc-bridge --exclude native_entry --exclude sentinel_system_monitor_node --exclude sentinel_controller_node --target thumbv7em-none-eabihf
